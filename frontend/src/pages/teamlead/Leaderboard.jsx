...
const TeamLeadLeaderboard = () => {
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      const mapped = res.data.map((intern, idx) => ({
        rank: idx + 1,
        name: intern.name,
        score: intern.totalMarks.toFixed(1),
        tasks: intern.submissions,
        reports: intern.submissions,
        trend: 0,
        avatar: intern.name.substring(0, 2).toUpperCase()
      }));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="space-y-3">
            {[leaderboard[1], leaderboard[2]].filter(Boolean).map((intern, idx) => (
              <div key={intern.rank} className="flex items-center p-4 bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all border cursor-pointer">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 truncate">{intern.name}</p>
                  <p className="text-sm text-gray-600">{intern.tasks} evals</p>
                </div>
